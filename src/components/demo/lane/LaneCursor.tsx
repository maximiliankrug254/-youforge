...
export function LaneCursor() {
  const fine = useLaneMediaQuery("(pointer: fine)");
  ...
  useEffect(() => {
    ...
    document.documentElement.classList.add("lane-cursor");
    ...
    const move = (e: PointerEvent) => {
      ...
      hover = Boolean(
        t?.closest(
          "a, button, input, textarea, select, label, [data-lane-hover]",
        ),
      );
    };
    ...
    return () => {
      ...
      document.documentElement.classList.remove("lane-cursor");
    };
  }, [fine, reduce]);
